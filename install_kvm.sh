...
sudo virt-install --name test --connect qemu:///system --ram 16384 --disk path=ubuntu.qcow2 --vcpus 16--os-type linux --network network=default --graphics none --console pty --location=ubuntu-16.04.6-server-amd64.iso --extra-args 'console=ttyS0,115200n8 serial'

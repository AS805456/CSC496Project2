...
--extra-args 'console=ttyS0' 'ks=file:/Kickstarter.cfg'
...
# sudo virt-install --name test1 --connect qemu:///system --ram 16384 --disk path=ubuntu.qcow2,format=qcow2,bus=virtio,size=8 --vcpus 8 --os-type linux --network network=default --graphics none --console pty,target_type=serial --location=ubuntu-16.04.6-server-amd64.iso --initrd-inject cluster-template/Kickstarter.cfg --extra_args 'ks=file:/Kickstarter.cfg' 'console=ttyS0'
